...
import { BASE_COLORS, type BaseKey, type Recipe, readableTextColor } from "@/lib/color-mix"
  ...
  return (
    <div>
      <div className="mb-3 flex items-baseline justify-between">
        <h2 className="font-mono text-xs uppercase tracking-widest text-muted-foreground">Palette</h2>
        <p className="text-xs text-muted-foreground">Click to add a part · right-click to remove</p>
      </div>
      <div className="grid grid-cols-3 gap-2 sm:grid-cols-5 lg:grid-cols-9">
        {BASE_COLORS.map((c) => {
          ...
          return (
            ...
            >
              <span className="self-start font-mono text-sm font-bold uppercase opacity-90">{c.key}</span>
              {active && (
                <span
                  ...
                  style={{ backgroundColor: readableTextColor(c.hex), color: c.hex }}
                >
                  {parts}
                </span>
              )}
              <span className="self-start text-[10px] font-medium leading-tight opacity-80">{c.name}</span>
            </button>
          )
        })}
      </div>
